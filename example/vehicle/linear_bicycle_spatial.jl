...
function linear_bicycle_sp(
    ...
    # -----------------------------
    t_nom = NDF(:-, [1.0, NDF(:*, [param.κ_c, e_y], s)], s)
    ...
    dξ = NDF(:-, [NDF(:*, [dψ, dt], s), param.κ_c], s)
    ...
    if starts == Interesso.WSS{DOI.AbstractDynamicSolution}()
        MOI.set(model, DOI.DynamicVariableStart(), t,   LinearInterpolant(0.0, 5.0,   s_0, s_f))
        MOI.set(model, DOI.DynamicVariableStart(), e_y, LinearInterpolant(0.0, 0.0,   s_0, s_f))
        MOI.set(model, DOI.DynamicVariableStart(), v_x, LinearInterpolant(vxi, 100.0, s_0, s_f))
